...
import { CustomDropdownCellNode } from '../../nodes/utils/CustomDropdownCellNode';
  ...
  return (
    <CellComponent
      className={clsx(
        ...
        isPrimarySelected && theme.tableCellSelected,
      )}
      data-id={cell.id}
      tabIndex={-1}
...
export default function TableComponent({
  ...
  useEffect(() => {
    if (!isEditing && primarySelectedCellID !== null) {
      ...
      const loadContentIntoCell = (cell: Cell | null, enterPressed = false) => {
        if (cell !== null && cellEditor !== null) {
          const editorState = cellEditor.parseEditorState(cell.json);
          cellEditor.setEditorState(editorState);
      ...
      const handleKeyDown = (event: KeyboardEvent) => {
        ...
        const enterIsPressed = keyCode === 13;

        loadContentIntoCell(cell, enterIsPressed);
        setIsEditing(true);
        setSelectedCellIDs(NO_CELLS);
      };
  ...
  return (
    <div style={{ position: 'relative' }}>
      <table
        className={clsx(theme.table, isSelected && theme.tableSelected)}
        ref={tableRef}
        tabIndex={-1}
      >
