...
export class TableNode extends DecoratorNode<ReactElement> {
  __rows: Rows;
  ...
  updateRowType(rowPos: number, type: 'header' | 'normal'): void {
    const self = this.getWritable();
    const rows = self.__rows;
    const row = rows[rowPos];

    for (let i = 0; i < row.cells.length; i++) {
      this.updateCellType(i, rowPos, type);
    }
  }

  updateColType(colPos: number, type: 'header' | 'normal'): void {
    const self = this.getWritable();
    const rows = self.__rows;
  ...
  decorate(_: LexicalEditor, config: EditorConfig): ReactElement {
    return (
      <Suspense>
        <TableComponent
          nodeKey={this.__key}
...
export function $createTableNodeWithDimensions(
  ...
  for (let y = 0; y < columnCount; y++) {
    ...
    for (let x = 0; x < rowCount; x++) {
      let headerState: 'normal' | 'header' = 'normal';

      if (typeof includeHeaders === 'object') {
        if (y === 0 && includeHeaders.rows) {
          headerState = 'header';
        }
        if (x === 0 && includeHeaders.columns) {
          headerState = 'header';
        }
      } else if (includeHeaders) {
        if (y === 0) headerState = 'header';
        if (x === 0) headerState = 'header';
      }

      row.cells.push(createCell(headerState));
    }
  }
  return new TableNode(rows);
}
