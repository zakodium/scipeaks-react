...
export interface VerticalNavigationGroupOption<T> {
  ...
  label: ReactNode;
  icon?: ReactNode;
  options: Array<VerticalNavigationOption<T>>;
}
...
function NavigationGroup<T>(props: NavigationGroupProps<T>): ReactElement {
  ...
  return (
    <div className="space-y-1">
      <button
        ...
        className={clsx(
          'group flex w-full items-center rounded-md bg-white py-2 pr-1 pl-2 hover:bg-neutral-50 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-primary-500',
          optionStyles[size],
        )}
      >
        {element.icon && (
          <div
            className={clsx(
              'size-6 text-2xl text-neutral-400 group-hover:text-neutral-600',
              iconStyles[size],
            )}
          >
            {element.icon}
        ...
        <svg
          className={clsx(
            'ml-auto size-5 transition-colors duration-150 ease-in-out group-hover:text-neutral-400',
...
function Navigation<T>(props: NavigationProps<T>): ReactElement {
  const isSelected =
  ...
  const item = (
    ...
    >
      {props.element.icon && (
        <div
          className={clsx(
            'size-6 group-hover:text-neutral-600',
