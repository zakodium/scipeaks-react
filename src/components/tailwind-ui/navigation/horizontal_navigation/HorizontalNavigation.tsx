...
export type HorizontalNavigationRenderNavigationItemCallback<
  T extends string,
  U extends HorizontalNavigationOption<T> = HorizontalNavigationOption<T>,
> = (option: U, index: number) => ReactElement;
...
> {
  options: U[];
  selected: U;
  onSelect: (clicked: U, options: U[]) => void;
}
...
function InternalHorizontalNavigation({
  ...
  selectedItem,
  bare,
}: InternalHorizontalNavigationProps): ReactElement {
...
function BaseOptionSmallScreen({ children }: BaseOptionScreenProps) {
  ...
  return (
    <div
      ...
      className={clsx(
        'cursor-pointer border-l-4 py-2 pr-4 pl-3 text-base font-medium transition duration-150 ease-in-out focus:outline-hidden',
        {
...
function BaseOptionLargeScreen({ children }: BaseOptionScreenProps) {
  ...
  return (
    <div
      ...
      className={clsx(
        'inline-block cursor-pointer px-1 py-4 text-sm font-semibold whitespace-nowrap',
      )}
    >
      {children}
    </div>
...
export function HorizontalNavigationItem<
  T extends string,
  U extends HorizontalNavigationOption<T> = HorizontalNavigationOption<T>,
>(props: HorizontalNavigationItemProps<T, U>): ReactElement {
  return (
    <>
      <div
        className={clsx(
...
function ShowNavButton({
  ...
  return (
    <IconButton
      ...
      className="inline-flex items-center justify-center rounded-md p-2 text-neutral-400 transition duration-150 ease-in-out hover:bg-neutral-100 hover:text-neutral-500 focus:bg-neutral-100 focus:text-neutral-500 focus:outline-hidden"
      icon={<Bars3Icon />}
      size="6"
      onClick={onCollapse}
    />
